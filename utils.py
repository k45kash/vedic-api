...
# ═══════════════════════════════════════════════════════════
#  НАКШАТРА И ПАДА ПО ДОЛГОТЕ
# ═══════════════════════════════════════════════════════════

# Ширина накшатры и пады в градусах. Ни 360/27, ни 360/108 не представимы
# в double, и это не теория: если считать номер накшатры и номер пады двумя
# независимыми делениями (`int(sid/NK)` и `int((sid % NK)/(NK/4))`), то на
# ТОЧНЫХ границах пад они расходятся — на 70 границах из 108, местами сразу
# на три пады (на 40.0° выходила Криттика пада 4 вместо пады 1).
#
# Для живых карт это ничего не значило: попасть в границу с точностью до
# 1e-15 практически невозможно. Но таблицы, которые строятся ПО границам —
# круг 108 пад, календарь входов Луны в накшатру (он бисекцией сходится
# ровно к границе) — попадают туда по построению.
#
# Поэтому номер пады считается ОДИН раз, и считается точно.
#
# «Умножить до деления» (sid * 108 / 360) чинит основную беду, но полностью
# от округления не спасает: на проверке в 10 тысяч точек такой вариант
# разошёлся с точной арифметикой в 23 из них. Поэтому берём Fraction —
# он представляет double как точную дробь, и результат перестаёт зависеть
# от порядка операций. Цена 1,8 мкс на вызов: на фоне обращения к эфемериде
# это ничто, а определение становится однозначным.
#
# Соглашение: пада — это полуинтервал [начало, конец), и вопрос «какая пада
# у ЭТОГО числа» имеет ровно один правильный ответ, без допусков.
NK_SIZE = 360.0 / 27
PADA_SIZE = NK_SIZE / 4

_PADA_WIDTH = Fraction(10, 3)          # 360/108 градуса


def global_pada(sid_lon: float) -> int:
    """Сквозной номер пады 0..107 от 0° Овна."""
    return int(Fraction(sid_lon) % 360 // _PADA_WIDTH)


def nakshatra_pada(sid_lon: float) -> tuple:
    """(индекс накшатры 0..26, номер пады 1..4) по сидерической долготе."""
    gp = global_pada(sid_lon)
    return gp // 4, gp % 4 + 1
