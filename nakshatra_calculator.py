...
def get_aya(jd: float, mode=None) -> float:
    """Аянамша **с нутацией** — та, что вычитается из видимой долготы.

    Тонкость, которая стоила нам систематического сдвига до 14″ по всем телам
    сразу. `swe.calc_ut` без FLG_SIDEREAL отдаёт ВИДИМУЮ тропическую долготу,
    то есть уже с нутацией. А `swe.get_ayanamsa_ut` отдаёт аянамшу БЕЗ неё —
    поэтому разность `trop - aya` считала нутацию дважды.

    Величина знакопеременная: +12.8″ на июнь 1990, −13.9″ на январь 2000.
    Астрологически это заметно только у самой границы накшатры или пады, но от
    всех прочих калькуляторов мы этим отличались, а `Panchangam.py` (он берёт
    FLG_SIDEREAL) расходился с этим модулем на ту же величину.

    `get_ayanamsa_ex_ut` возвращает (флаг, аянамша) и учитывает нутацию, так
    что `trop - aya` теперь совпадает с FLG_SIDEREAL до микроградуса.
    """
    swe.set_sid_mode(mode if mode is not None else swe.SIDM_LAHIRI)
    return swe.get_ayanamsa_ex_ut(jd, swe.FLG_SWIEPH)[1]   # _ut принимает UT напрямую, без ΔT
